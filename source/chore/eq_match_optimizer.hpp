...
namespace zlchore::eq_match {
    ...
    public:
        EqMatchOptimizer(const double sample_rate,
            ...
            zldsp::vector::multiply(diffs_.data(), diffs.data(),
                                    static_cast<float>(std::log(10) * 0.1), diffs.size());

            const auto w_scale = static_cast<float>(2 * std::numbers::pi / sample_rate);
            ws_.resize(freqs.size());
            zldsp::vector::multiply(ws_.data(), freqs.data(), w_scale, ws_.size());
        ...
        size_t fit(std::vector<zldsp::filter::FilterParameters>& paras,
            ...
            paras.clear();
            const double sum_sqr = zldsp::vector::sum_sqr(diffs_.data(), diffs_.size());
            if (sum_sqr < kEps) {
                return 0;
            }
        ...
        std::tuple<zldsp::filter::FilterParameters, double> addOneFilter(
...

            {
                ...
                size_t i = 0;
                for (; i + lanes <= diffs_.size(); i += lanes) {
                    const auto v_diff = hn::Load(d, diffs_.data() + i);
                    const auto v_res  = hn::Load(d, res_.data() + i);
                    const auto v_log = hn::Log(d, v_res);
                    const auto v_result = hn::Sub(v_diff, v_log);
                    hn::Store(v_result, d, diffs_.data() + i);
                }
                for (; i < diffs_.size(); ++i) {
    ...
    private:
        ...
        struct OptFData {
            size_t n;
            zldsp::filter::Ideal<float, 6>* filter;
            float* ws;
            float* diffs;
            float* res;
        };
        ...
        static double calculateMSE(const std::span<double> x, const OptFData* data) {
            ...
            double sum_sqr = 0.0;
            zldsp::vector::log<float, true>(data->res, data->n);
            for (size_t i = 0; i < data->n; ++i) {
                const auto err = data->diffs[i] - data->res[i];
                sum_sqr += static_cast<double>(err * err);
            }
