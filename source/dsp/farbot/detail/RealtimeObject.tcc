...
namespace farbot {
    namespace detail {
        ...
        public:
            ...
            explicit NonRealtimeMutatable(T &&obj) : storage(std::make_unique<T>(std::move(obj))),
                                                     pointer(storage.get()) {
            }

            ~NonRealtimeMutatable() {
                assert(pointer.load() != nullptr);
                // <- never delete this object while the realtime thread is holding the lock
                ...
                ((void) (accquired));
                assert(accquired);
                // <- you didn't call release on one of the non-realtime threads before deleting this object
        ...
        public:
            ...
            ~RealtimeMutatable() {
                assert((control.load() & BUSY_BIT) == 0);
                // <- never delete this object while the realtime thread is still using it
            ...
            void realtimeRelease() noexcept {
                ...
                data[static_cast<size_t>(idx)] = realtimeCopy;
                releaseIndex(idx);
            }
            ...
            const T &nonRealtimeAcquire() {
                ...
                return data[static_cast<size_t>(nonRealtimeIndex)];
            }
        ...
        private:
            ...
            explicit RealtimeMutatable(bool, Args &&... args)
                : data({
                  ...
                  realtimeCopy(std::forward<Args>(args)...) {
            }
