...
namespace zldsp::filter {
    constexpr double sqr(const double x) {
        return x * x;
    }

    constexpr double get2TiltShelfMagnitude2Fast(const double w0_over_q, const double w02,
                                                 const double sqrt_g, const double ssqrt_g,
                                                 const double w) {
        return AnalogFunc::get2Magnitude2({1, ssqrt_g * w0_over_q, sqrt_g * w02,
                                           sqrt_g, ssqrt_g * w0_over_q, w02}, w);
    }
    ...
    std::array<double, 4> MartinCoeff::get1LowPass(const double w0) {
        ...
        const auto alpha = -2 * a1 / sqr(1 + a1);
    ...
    std::array<double, 4> MartinCoeff::get1HighPass(const double w0) {
        ...
        const std::array<double, 2> phim = {1 - sqr(std::sin(wm / 2)), sqr(std::sin(wm / 2))};

        const auto a1 = -std::exp(-w0);

        const std::array<double, 2> A = {sqr(1.0 + a1), sqr(1.0 - a1)};
    ...
    std::array<double, 4> MartinCoeff::get1TiltShelf(const double w0, const double g) {
        ...
        const auto phim = 1 - std::cos(kPi * fm);
        const auto alpha = 2 / kPi2 * (1 / sqr(fm) + 1 / g / sqr(fc)) - 1 / phim;
        const auto beta = 2 / kPi2 * (1 / sqr(fm) + g / sqr(fc)) - 1 / phim;
    ...
    std::array<double, 6> MartinCoeff::get2Peak(const double w0, const double g, const double q) {
        const auto a = solve_a(w0, 0.5 / std::sqrt(g) / q);
        const auto A = get_AB(a);
        const auto phi0 = get_phi(w0);

        const auto R1 = dotProduct(A, phi0) * sqr(g);
        const auto R2 = (-A[0] + A[1] + 4 * (phi0[0] - phi0[1]) * A[2]) * sqr(g);

        std::array<double, 3> B{A[0], 0, 0};
        B[2] = (R1 - R2 * phi0[1] - B[0]) / (4 * sqr(phi0[1]));
    ...
    std::array<double, 6> MartinCoeff::get2TiltShelf(const double w0, double g, const double q) {
        ...
        const auto a = solve_a(w0, ssqrt_g / 2 / q, ssqrt_g);
        const auto A = get_AB(a);

        const auto c2 = sqrt_g * (-1 + 2 * q * q);
        const auto c2half_rep = 1.0 / (2.0 * c2);
        const auto w02 = w0 * w0;
        const auto w0_over_q = w0 / q;
        const auto c0 = c2 * w02 * w02;
        const auto c1 = -2 * (1 + g) * sqr(q * w0);
        ...
        while (!check_AB(B) && trial < 20) {
            ...
            for (size_t i = 0; i < 3; ++i) {
                phi[i] = get_phi(ws[i]);
                res[i] = get2TiltShelfMagnitude2Fast(w0_over_q, w02, sqrt_g, ssqrt_g, ws[i]) * dotProduct(phi[i], A);
            }
            B = linear_solve(phi, res);
            ws[2] = 0.5 * (ws[2] + kPi);
        }
    ...
    std::array<double, 3> MartinCoeff::solve_a(const double w0, const double b) {
        std::array<double, 3> a{};
        a[0] = 1.0;
        const auto t = std::exp(-b * w0);
        if (b <= 1.0) {
            a[1] = -2 * t * std::cos(std::sqrt(1.0 - b * b) * w0);
        } else {
            a[1] = -2 * t * std::cosh(std::sqrt(b * b - 1.0) * w0);
        }
        a[2] = t * t;
        return a;
    }
    ...
    std::array<double, 3> MartinCoeff::solve_a(const double w0, const double b, const double c) {
        ...
        const auto t = std::exp(-b * w0);
        if (b <= c) {
            a[1] = -2 * t * std::cos(std::sqrt(c * c - b * b) * w0);
        } else {
            a[1] = -2 * t * std::cosh(std::sqrt(b * b - c * c) * w0);
        }
        a[2] = t * t;
        return a;
    }
    ...
    bool MartinCoeff::check_AB(const std::array<double, 3>& A) {
        return A[0] > 0 && A[1] > 0 && sqr(0.5 * (std::sqrt(A[0]) + std::sqrt(A[1]))) + A[2] > 0;
    }
    ...
    std::array<double, 3> MartinCoeff::get_ab(const std::array<double, 3>& A) {
        ...
        const auto W = 0.5 * (A0 + A1);
        const auto temp = std::max(W * W + A[2], 0.0);
    ...
    std::array<double, 3> MartinCoeff::get_phi(const double w) {
        const double c = 0.5 * std::cos(w);
        const double p0 = 0.5 + c;
        const double p1 = 0.5 - c;
        return {p0, p1, 4.0 * p0 * p1};
    }
    ...
    std::array<double, 3> MartinCoeff::linear_solve(const std::array<std::array<double, 3>, 3>& A,
        ...
        if (std::abs(A[0][0]) > std::abs(A[0][1])) {
            x[0] = b[0] / A[0][0];
            const auto denominator = -(A[1][2] * A[2][1] - A[1][1] * A[2][2]);
            ...
            x[1] = b[0] / A[0][1];
            const auto denominator = -(A[1][2] * A[2][0] - A[1][0] * A[2][2]);
