    ...
    public:
        ...
        void process(std::span<FloatType *> buffer, const size_t num_samples) {
            if (this->c_freq_.isSmoothing() || this->c_gain_.isSmoothing() || this->c_q_.isSmoothing()) {
                processSVF<IsBypassed, true>(buffer, num_samples);
            } else {
                processSVF<IsBypassed, false>(buffer, num_samples);
            }
        }
         ...
         * @return
         */
        std::array<SVFBase<FloatType>, FilterSize> &getFilters() {
            return filters_;
        }

    private:
        std::array<SVFBase<FloatType>, FilterSize> filters_{};
